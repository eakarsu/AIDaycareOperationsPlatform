-- Additive migration: tenant-scoped, approval-gated care_case workflow.
...
CREATE TABLE IF NOT EXISTS governed_cases (
  ...
  case_kind VARCHAR(64) NOT NULL CHECK (case_kind = 'care_case'),
  business_key VARCHAR(160) NOT NULL,
  state VARCHAR(64) NOT NULL CHECK (state IN ('draft', 'consent_verified', 'scheduled', 'active', 'incident_hold', 'escalation_pending', 'escalated', 'resolved', 'closed')),
...
CREATE TABLE IF NOT EXISTS governed_integration_runs (
  id BIGSERIAL PRIMARY KEY,
  tenant_id VARCHAR(128) NOT NULL,
  connector VARCHAR(64) NOT NULL CHECK (connector IN ('care_provider', 'calendar', 'messaging', 'billing', 'emergency_contacts', 'health_feed')),
...
CREATE TABLE IF NOT EXISTS governed_quality_observations (
  ...
  tenant_id VARCHAR(128) NOT NULL,
  case_id BIGINT REFERENCES governed_cases(id) ON DELETE RESTRICT,
  metric_name VARCHAR(80) NOT NULL CHECK (metric_name IN ('schedule_coverage', 'handoff_completeness', 'notification_delivery', 'accessibility', 'incident_response_time', 'emergency_readiness')),
