...
/**
 * Gap/CFS Feature Page: `attendance` lacks AI no-show prediction.
 * Slug: attendance-lacks-ai-no-show-prediction
 * Backend: /api/gap-attendance-lacks-ai-no-show-prediction
 */
export default function AttendanceLacksAiNoShowPredictionPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-attendance-lacks-ai-no-show-prediction', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>`attendance` lacks AI no-show prediction.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>`attendance` lacks AI no-show prediction.</p>
