...
import { FaUserPlus, FaPlus, FaEdit, FaTrash } from 'react-icons/fa';
import Modal from '../components/Modal';

const API = '/api/enrollment';
const headers = () => ({ Authorization: `Bearer ${localStorage.getItem('token')}` });
const emptyForm = { child_name: '', parent_name: '', parent_email: '', parent_phone: '', date_of_birth: '', age_group: '', enrollment_date: '', status: 'active', classroom: '', notes: '' };
...
const Enrollment = () => {
  ...
  const openEdit = () => { setForm({ ...emptyForm, ...selected, date_of_birth: selected.date_of_birth ? selected.date_of_birth.split('T')[0] : '', enrollment_date: selected.enrollment_date ? selected.enrollment_date.split('T')[0] : '' }); setEditing(true); setShowDetail(false); setShowForm(true); };
  ...
  const statusBadge = (status) => {
    const map = { active: 'badge-success', waitlisted: 'badge-warning', withdrawn: 'badge-danger', graduated: 'badge-info' };
  ...
  return (
    <div className="animate-in">
      <div className="page-toolbar">
        <h1><FaUserPlus style={{ marginRight: 10, color: '#43A047' }} />Enrollment Management</h1>
        <button className="btn btn-primary" onClick={openCreate}><FaPlus /> New Enrollment</button>
      </div>

      <div className="table-container">
        <table className="data-table">
          <thead><tr><th>Child Name</th><th>Parent</th><th>Age Group</th><th>Classroom</th><th>Enrollment Date</th><th>Status</th></tr></thead>
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan="6" className="table-empty"><p>No enrollment records found.</p></td></tr>
            ) : items.map(item => (
              <tr key={item.id} onClick={() => openDetail(item)}>
                ...
                <td>{item.enrollment_date ? new Date(item.enrollment_date).toLocaleDateString() : 'N/A'}</td>
      ...
      <Modal isOpen={showDetail} onClose={() => setShowDetail(false)} title="Enrollment Details"
        ...
        {selected && (
          <div className="detail-grid">
            ...
            <div className="detail-item"><div className="detail-label">Enrollment Date</div><div className="detail-value">{selected.enrollment_date ? new Date(selected.enrollment_date).toLocaleDateString() : 'N/A'}</div></div>
      ...
      <Modal isOpen={showForm} onClose={() => setShowForm(false)} title={editing ? 'Edit Enrollment' : 'New Enrollment'}
        ...
        <form onSubmit={handleSave}>
          ...
          <div className="form-row">
            <div className="form-group"><label>Enrollment Date</label><input className="form-control" type="date" name="enrollment_date" value={form.enrollment_date} onChange={onChange} /></div>
            <div className="form-group"><label>Status</label>
              <select className="form-control" name="status" value={form.status} onChange={onChange}>
                <option value="active">Active</option><option value="waitlisted">Waitlisted</option><option value="withdrawn">Withdrawn</option><option value="graduated">Graduated</option>
              </select>
            </div>
          </div>
